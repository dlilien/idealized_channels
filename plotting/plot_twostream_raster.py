...
setup = ts_name
full_fn = chan_fn_template.format(setup, "channels", "full")
...
inner_fn = chan_fn_template.format(setup, "channels", "inner")
outer_fn = chan_fn_template.format(setup, "channels", "outer")
...
with firedrake.CheckpointFile(funky_fn, "r") as chk:
    fine_mesh = chk.load_mesh(name="fine_mesh")
    for f in ["thickness", "velocity", "surface", "tvm"]:
        funky_dict_even[f] = chk.load_function(
            fine_mesh,
            "{:d} {:d} {:s} even".format(key_funky[0], key_funky[1], f),
        )
        funky_dict_chans[f] = chk.load_function(
            fine_mesh,
            "{:d} {:d} {:s} chan".format(key_funky[0], key_funky[1], f),
        )
    v_funky = chk.get_attr("{:d} {:d}".format(*key_funky), "volume")
...
even_dicts = [full_dict_even, margin_dict_even, funky_dict_even, center_dict_even, inner_dict_even, outer_dict_even]
chans_dicts = [full_dict_chans, margin_dict_chans, funky_dict_chans, center_dict_chans, inner_dict_chans, outer_dict_chans]
A = firedrake.Constant(20)

vols = np.array([v_full, v_margin, v_funky, v_center])
print(vols)
print((vols - vols[0]) / vols[0] * 100)


def four_column(even_dicts, chans_dicts):
    ...
    for h_ax, u_ax, du_ax, ss_ax, even_dict, chan_dict in zip(h_axes, u_axes, du_axes, ss_axes, even_dicts, chans_dicts):
        ...
        cm_du = tripcolor(extract_surface(du), vmin=-400, vmax=400, cmap="PuOr", axes=du_ax)
        ...
        if False:
            if ax in du_axes:
                for pt, marker in zip(all_pts, pt_symbols):
                    ax.plot(*pt, marker=marker, linestyle="none", color="0.2")
        if ax in ss_axes:
    ...
    fig.savefig("../plots/hybrid/twostream_channel_basics.png", dpi=300)


def singles(even_dicts, chans_dicts):
    ...
    for h_ax, u_ax, du_ax, ss_ax, even_dict, chan_dict in zip(h_axes, u_axes, du_axes, ss_axes, even_dicts[1:], chans_dicts[1:]):
        Q2 = even_dict["thickness"].function_space()
        du = firedrake.project(
            firedrake.assemble(interpolate(chan_dict["velocity"][0], Q2)) - firedrake.assemble(interpolate(even_dict["velocity"][0], Q2)),
            Q2,
    ...
    fig.savefig("../plots/hybrid/twostream_singles.png", dpi=300)


def pairs(even_dicts, chans_dicts):
    gs = gridspec.GridSpec(4, 4, width_ratios=(1, 1, 1, 0.04), top=0.98, left=0.075, wspace=0.10, right=0.905, bottom=0.12)
    fig = plt.figure(figsize=(7.0, 3.2))
    h_axes = [fig.add_subplot(gs[0, 0]), fig.add_subplot(gs[0, 1]), fig.add_subplot(gs[0, 2])]
    ...
    for h_ax, u_ax, du_ax, ss_ax, even_dict, chan_dict in zip(h_axes, u_axes, du_axes, ss_axes, [even_dicts[0]] + even_dicts[4:], [chans_dicts[0]] + chans_dicts[4:]):
        Q2 = even_dict["thickness"].function_space()
        du = firedrake.project(
            firedrake.assemble(interpolate(chan_dict["velocity"][0], Q2)) - firedrake.assemble(interpolate(even_dict["velocity"][0], Q2)),
            Q2,
